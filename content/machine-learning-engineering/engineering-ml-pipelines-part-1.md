Title: Best Practices for Engineering ML Pipelines - Part 1
Date: 2021-03-03
Tags: python, machine-learning, mlops, kubernetes, bodywork

![ml-pipeline-engineering]({static}/images/machine-learning-engineering/ml-pipeline-engineering/pipelines-logo.png)
...
- `.circleci/config.yml` contains the configuration for the project’s CI/CD pipeline (using [CircleCI](https://circleci.com)). We'll discuss in more depth later on.
...
- `flake8.ini` & `mypy.ini` are configuration files for [Flake8](https://flake8.pycqa.org/en/latest/#) code style enforcement and [MyPy](https://mypy.readthedocs.io/en/stable/) static type checking.
- `tox.ini` provides configuration for the [Tox](https://tox.readthedocs.io/en/latest/index.html) test automation framework.
- `bodywork.yaml` is the [Bodywork](https://bodywork.readthedocs.io/en/latest/) deployment configuration file.
